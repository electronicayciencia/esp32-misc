...
void app_main(void)
{
    ...
    // Use standard Philips I2S mode (LRCK = sample rate, BCLK = 64x)
    i2s_std_config_t std_cfg = {
        .clk_cfg = {
            .sample_rate_hz = SAMPLE_RATE,
            .clk_src = I2S_CLK_SRC_APLL,  // Only on ESP32!
    ...
    // Ensure slot bit width matches data width
    std_cfg.slot_cfg.slot_bit_width = I2S_SLOT_BIT_WIDTH_16BIT;

    ESP_ERROR_CHECK(i2s_channel_init_std_mode(tx_handle, &std_cfg));
    ESP_ERROR_CHECK(i2s_channel_enable(tx_handle));

    // Send silent audio forever
    uint16_t zeros[128 * 2] = {0}; // stereo: L, R, L, R...
    while (1) {
